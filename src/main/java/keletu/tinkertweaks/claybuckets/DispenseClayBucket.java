...
public class DispenseClayBucket extends BehaviorDefaultDispenseItem {
	...
	public ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
		...
		ItemClayBucket.SpecialFluid fluid = IguanaItems.clayBucketFired.getSpecialFluid(stack);
		...
		if(!IguanaItems.clayBucketFired.hasFluid(stack)/* && Config.bucketSand*/) {
			ItemClayBucket.SpecialFluid newFluid = ItemClayBucket.SpecialFluid.fromState(state);
			if(newFluid != null) {
				// sound
				...
				ItemStack filled = IguanaItems.clayBucketFired.withSpecialFluid(newFluid);
				stack.shrink(1);
