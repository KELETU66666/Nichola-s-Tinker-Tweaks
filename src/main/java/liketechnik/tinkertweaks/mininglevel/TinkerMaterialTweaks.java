...
import static liketechnik.tinkertweaks.util.HarvestLevels.*;
...
public final class TinkerMaterialTweaks {
    ...
    public static void modifyToolMaterials()
    {
        ...
        durabilityMod = Config.getDurabilityPercentage() / 100f;
        speedMod = Config.getMiningSpeedPercentage() / 100f;

        // at first we update each material to keep the harvest levels on non-listed materials appropriate
        Collection<Material> copyOfMaterials = new HashSet<Material>(TinkerRegistry.getAllMaterials());
        for(Material material : copyOfMaterials) {
            HeadMaterialStats stats = material.getStats(material.getIdentifier());
            HandleMaterialStats statsHandle = material.getStats(material.getIdentifier());
    ...
    private static void modifyTcon()
    {
        ...
        updateMaterial(_6_obsidian, "knightslime", 550, 790, 4, 1.3f); // Good All around material
    ...
    {
        //TODO: Keletu:finish those compat.
...
//
    ...
    {
        Material old = TinkerRegistry.getMaterial(matName);
        if(old == null)
        {
        ...
        // round everything
        Material newMaterial = new Material(old.identifier, old.materialTextColor);

        Field rwkk, rwkk1;

        rwkk = ReflectionHelper.findField(TinkerRegistry.class, "materials");
        rwkk.setAccessible(true);

        rwkk1 = ReflectionHelper.findField(Material.class, "stats");
        rwkk1.setAccessible(true);

        // replace old material
        if(TinkerRegistry.getAllMaterials().contains(old)) {
            try {
                ((Map<String, IMaterialStats>)rwkk1.get(old)).values().clear();
            } catch (IllegalAccessException e) {
                throw new RuntimeException(e);
            }
            TinkerRegistry.addMaterialStats(old, new HeadMaterialStats(durability, speed, newAttack, harvestLevel), new HandleMaterialStats(newHandleModifier, durability / 2), new ExtraMaterialStats(durability / 2));
        }
        else
            Log.error("Couldn't find ToolMaterial ID for " + old.identifier);
    }
}